...
router.get("/verify/:qrCode", verifyToken, async (req, res) => {
  try {
    ...
    console.log(
      `Verifying medicine with QR code: ${qrCode} by ${req.user.username}`
    );
    ...
    try {
      ...
      const allMedicinesResult = await contract.evaluateTransaction(
        "GetAllMedicines"
      );
      ...
      if (!medicine) {
        ...
        return res
          .status(404)
          .json({ error: `Medicine with QR code ${qrCode} not found` });
      }
      ...
      console.log(
        `Scan authorization: ${isAuthorized ? "Authorized" : "UNAUTHORIZED"}`
      );

      // For unauthorized scans, flag the medicine automatically
      if (!isAuthorized) {
        console.warn(
          `SECURITY ALERT: Unauthorized scan detected - User: ${req.user.username}, Role: ${req.user.role}, Org: ${req.user.organization}, Medicine: ${medicineData.id}`
        );
        ...
        try {
          ...
          console.log(
            `Medicine ${medicineData.id} auto-flagged due to unauthorized scan`
          );
        } catch (flagError) {
        ...
        try {
          if (
            ...
            try {
              ...
              const updatedResult = await contract.evaluateTransaction(
                "GetMedicine",
                medicineData.id
              );
              ...
              console.log(
                `Scan recorded for medicine ${medicineData.id} by ${req.user.username}`
              );
            } catch (error) {
  ...
  if (user.role === "distributor") {
    return (
      ...
      (medicine.status === "In Transit" &&
        medicine.supplyChain.some(
          (entry) => entry.notes && entry.notes.includes(user.organization)
        ))
    );
  }
  ...
  try {
    ...
    if (req.user.role !== "regulator" && req.user.organization !== owner) {
      return res.status(403).json({
        error:
          "You are not authorized to view medicines from other organizations",
      });
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const secretKey =
        process.env.QR_SECRET_KEY || "farmatech-secure-key-2025";
...
router.post("/verify-secure", verifyToken, async (req, res) => {
  try {
    ...
    console.log(
      `Verifying medicine with secure QR code by ${req.user.username}`
    );
    ...
    try {
      ...
      console.log(
        `Found medicine: ${medicine.id} with QR code: ${blockchainQR}`
      );
...
router.get("/manufacturer/:manufacturer", verifyToken, async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "regulator" &&
      req.user.organization !== manufacturer
    ) {
      return res.status(403).json({
        error:
          "You are not authorized to view medicines from other manufacturers",
      });
    }
    const ccpPath = path.resolve(
      __dirname,
    ...
    console.log(
      `Calling GetMedicinesByManufacturer with param: ${manufacturer}`
    );
    const result = await contract.evaluateTransaction(
      "GetMedicinesByManufacturer",
      manufacturer
    ...
    const medicines = JSON.parse(result.toString());
    console.log(
      `Found ${medicines.length} medicines for manufacturer ${manufacturer}`
    );
  ...
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(distributors) || distributors.length === 0) {
        return res
          .status(400)
          .json({ error: "Distributors must be a non-empty array" });
      }
      ...
      try {
        ...
        const medicineData = await contract.evaluateTransaction(
          "GetMedicine",
          id
        );
        
        const medicine = JSON.parse(medicineData.toString());
        console.log("Medicine manufacturer:", medicine.manufacturer);
        console.log("Expected MSP ID:", medicine.manufacturer + "MSP");

        const medicineResult = await contract.evaluateTransaction(
          "GetMedicine",
          id
        );
        ...
        if (medicine.manufacturer !== req.user.organization) {
          ...
          return res.status(403).json({
            error:
              "Only the manufacturer can assign distributors to this medicine",
          });
        }
        ...
        for (const distributorOrg of distributors) {
          try {
            ...
            console.error(
              `Error sending notification to ${distributorOrg}:`,
              notificationErr
            );
          }
        }
