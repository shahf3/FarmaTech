...
const DistributorDashboard = () => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      // Check if medicine was flagged due to unauthorized scan
      if (medicine.flagged) {
        setScanResult({
          success: false,
          message: "Unauthorized access detected! Medicine has been flagged.",
          type: "error",
        });
      } else {
        setScanResult({
              ...
              <div className="dashboard-section">
                ...
                {verifiedMedicine && verifiedMedicine.flagged && (
                  <div className="security-warning">
                    <h4>🚨 Security Alert: Unauthorized Access Detected 🚨</h4>
                    {verifiedMedicine.unauthorizedScanDetails && (
                      <div className="unauthorized-scan-details">
                        <p>
                        ...
                        <p>
                          <strong>Role:</strong>{" "}
                          {verifiedMedicine.unauthorizedScanDetails.scannerRole}
                        </p>
                        <p>
                          <strong>Location:</strong>{" "}
                          {verifiedMedicine.unauthorizedScanDetails.location}
                        </p>
                        <p>
                          <strong>Timestamp:</strong>{" "}
                          {new Date(
                            verifiedMedicine.unauthorizedScanDetails.timestamp
                          ).toLocaleString()}
                        </p>
                      </div>
                    )}
                    <p>
                      This medicine has been automatically flagged due to
                      unauthorized access.
                    </p>
                  </div>
                )}
