// src/components/RegisterRegulator.js
...
const RegisterRegulator = () => {
  ...
  useEffect(() => {
    console.log('RegisterRegulator themeMode:', themeMode);
  }, [themeMode]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(
        `Regulator ${formData.username} registered successfully! An email with login credentials has been sent to ${formData.email}`
      );
  ...
  return (
    <div className="dashboard-wrapper" style={{ background: 'transparent' }}>
      <div
        className={`dashboard-section register-regulator-section ${themeMode === 'dark' ? 'dark-mode' : ''}`}
        style={{ backgroundColor: themeMode === 'dark' ? '#2a2a2a' : '#ffffff' }}
        data-theme={themeMode}
      >
        <h2 style={{ color: themeMode === 'dark' ? '#ffffff' : '#222222' }}>
          Register New Regulator
        </h2>
