// src/components/dashboard/RegulatorInventory.js
...
const getStatusColor = (status) => {
  const statusColors = {
    ...
    Dispensed: "#8bc34a",
    Flagged: "#f44336",
    "Order Complete": "#1976d2",
    Claimed: "#388e3c",
  };
  return statusColors[status] || "#757575";
};

const RegulatorInventory = () => {
  ...
  const handleOpenNotifyDialog = () => {
    handleMenuClose();
    setNotifyForm({
      subject: `Compliance Issue: ${selectedMedicine.name} (${selectedMedicine.id})`,
      message: "",
    });
    ...
    try {
      const response = await axios.post(
        `${API_URL}/medicines/${selectedMedicine.id}/update`,
        {
          ...
          notes: updateForm.notes || `Updated by Regulator: ${user.organization}`,
        },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const getAvailableStatusOptions = (currentStatus) => {
    if (["Order Complete", "Claimed"].includes(currentStatus)) {
      return [];
    }
    return ["Approved", "Flagged", "Order Complete"];
  };

  return (
    <div className="regulator-inventory">
      <InventoryContainer>
        <Typography variant="h5" component="h1" gutterBottom>
          Medicine Compliance Inventory
        </Typography>

        <Typography variant="body1" color="text.secondary" paragraph>
          Oversee medicines in the supply chain, approve compliant medicines, or flag issues for further investigation.
        </Typography>
        ...
        {loading ? (
          ...
          <Grid container spacing={3}>
            {filteredMedicines.map((medicine) => {
              ...
              const isLocked = ["Order Complete", "Claimed"].includes(medicine.status);
                  ...
                  >
                    <CardContent>
                      ...
                      <Button
                        ...
                        disabled={medicine.flagged || isLocked}
                      >
                        Update Status
                      </Button>
                    </CardContent>
      ...
      >
        <MenuItem
          ...
          disabled={selectedMedicine && ["Order Complete", "Claimed"].includes(selectedMedicine.status)}
        >
      ...
      <Dialog
        ...
        maxWidth="sm"
      >
        <DialogTitle>Update Medicine Compliance Status</DialogTitle>
        <DialogContent>
          {selectedMedicine && (
            <>
              ...
              <FormControl fullWidth margin="normal">
                <TextField
                  ...
                  multiline
                  rows={3}
                  placeholder="Add any additional details about this compliance update"
                />
              </FormControl>
            </>
          )}
            ...
            <>
              ...
              <FormControl fullWidth margin="normal">
                <TextField
                  ...
                  multiline
                  rows={3}
                  placeholder="Describe the compliance issue with this medicine"
                />
              </FormControl>
            ...
            <>
              ...
              <FormControl fullWidth margin="normal">
                <TextField
                  ...
                  multiline
                  rows={4}
                  placeholder="Enter your compliance message to the manufacturer"
                />
              </FormControl>
            </>
          )}
...
export default RegulatorInventory;